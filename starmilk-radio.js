(() => {
  ...
  const tryDiscover = async () => {
    ...
    try {
      ...
      // FIX 5: 15s hard cleanup — iframe never leaks if READY never fires
      const cleanup = setTimeout(() => { try { tmp.remove(); } catch (_) {} }, 15000);
      ...
      w.bind(SC.Widget.Events.READY, () => {
        w.getSounds((sounds) => {
          ...
          // FIX 2 cont: restore by URL not name
          const playingUrl = allTracks[currentIndex]?.url || null;
          allTracks = [...newTracks, ...allTracks];

          if (playingUrl) {
            const restored = allTracks.findIndex(t => t.url === playingUrl);
            if (restored !== -1) currentIndex = restored;
          }
          ...
          if (deepLinkedTrack) {
            const idx = allTracks.findIndex(t => slugify(t.name) === deepLinkedTrack);
            if (idx !== -1) { currentIndex = idx; deepLinkedTrack = null; swapTrack(false); }
          }
        });
      });
    } catch (_) {}
  ...
  const trackGradient = (name) => {
    ...
    const hue1 = ((h & 0xFF) / 255 * 60) + 240;
    const hue2 = (((h >> 8) & 0xFF) / 255 * 40) + 20;
  ...
  const buildQueue = () => {
    ...
    filteredTracks.forEach((track) => {
      ...
      heartBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleFav(track); });
  ...
  // FIX 4: rebuild from live state — no DOM name-match walk
